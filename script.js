...
function card() {
  ...
  testimonial.innerText = testimonials[user].text;
  userImage.src = testimonials[user].photo;
  userName.innerText = testimonials[user].name;
  role.innerText = testimonials[user].position;

  user++;
  
  if (user > testimonials.length - 1) {
    user = 0;
  }
